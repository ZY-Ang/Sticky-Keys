...
public class Player2Controller : MonoBehaviour {
    ...
    public Rigidbody2D player1Body;
    ...
    void Start()
    {
        onFloor = false;
		isDead = false;
        player2Body = GetComponent<Rigidbody2D>();
		Physics2D.IgnoreCollision(player1Body.GetComponent<Collider2D>(), GetComponent<Collider2D>());
    }
    ...
    {
        bool A = Input.GetKey(KeyCode.LeftArrow);
        bool D = Input.GetKey(KeyCode.RightArrow);
        bool W = Input.GetKey(KeyCode.UpArrow);
        ...
        if (W && onFloor)
        {
            player2Body.AddForce(new Vector2(0, speed * gravity));
            onFloor = false;
    ...
    private void OnCollisionStay2D(Collision2D other)
    {
        ...
        if (other.collider.gameObject.CompareTag("hazard1") && invulnerability == 0)
        {
            livesRemaining -= 1;
            invulnerability = 3;
            player2Body.AddForce(new Vector2(0, speed * gravity * 5f));
            timeHit = Time.time;
		...
		if (other.collider.gameObject.CompareTag("powerup3"))
		{
			...
			shiftScript.swapLine.SetPosition(0, player1.transform.position);
        ...
        {
			if (!isDead) {
				GameObject.Find ("Death").GetComponent<AudioSource> ().pitch = 0.7f;
	...
	public void resetPosition() {
		if (player2Body.position.y < -11 || player2Body.position.x > 38 || player2Body.position.x < -38)
			player2Body.position = new Vector3(20, -6.5f, 0);
	}
}
